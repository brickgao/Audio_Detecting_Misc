function [out_PCM_samples frequency] = decoder(file_name, output_file_name)
                    ...
                    for gr = 1:2
                        for ch= 1:2                            
                            ...
                            if ( part2_3_length(ri) == 0 )
                                ...
                                %TODO check It's right to output MDCT coefficients

                                tmp_bits = huffman_decoded_bits(ri,:)
                                [m, n] = size(tmp_bits)
                                for i = 1 : m
                                    for j = 1 : n
                                        fprintf(fout, '%d ', tmp_bits(i,j))
                                    end
                                    fprintf(fout, '\n')
                                end
                                fprintf(fout, '\n')
